...
export const TransactionHistory: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        // Look back 10000 blocks (approximately 1-2 days on most chains)
        ...
        for (const event of sentEvents) {
          if ('args' in event) {
            ...
            processedTxs.push({
              ...
              type: getTransactionType(event.args[0], event.args[1], user.address) as 'sent' | 'received' | 'minted'
            });
          }
        }
  ...
  const getTransactionType = (from: string, to: string, userAddress: string) => {
    ...
    if (from.toLowerCase() === zeroAddress.toLowerCase() && to.toLowerCase() === userAddress.toLowerCase()) {
      return 'minted';
    } else if (from.toLowerCase() === userAddress.toLowerCase() && to.toLowerCase() === zeroAddress.toLowerCase()) {
      return 'sold';
      ...
      {transactions.length === 0 ? (
        ...
        <div>
          {transactions.map((tx) => {
            ...
            return (
              <div key={tx.hash} className={`transaction-entry ${typeConfig.bgColor}`}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${typeConfig.color} bg-white shadow-sm`}>
                  ...
                  <span className="font-bold text-lg">{formatValue(tx.value)}</span>
                </div>
                {typeConfig.addressLabel && (
                  <div className="text-sm text-gray-600 pl-13">
                    <p>{typeConfig.addressLabel} {formatAddress(tx.type === 'sent' ? tx.to : tx.from)}</p>
                  </div>
                )}
                <a
